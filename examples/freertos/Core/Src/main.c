...
const osThreadAttr_t tickTask_attributes = { .name = "tickTask", .stack_size =
...
/* Definitions for wasmCtlTask */
osThreadId_t wasmCtlTaskHandle;
const osThreadAttr_t wasmCtlTask_attributes = { .name = "wasmCtlTask",
        .stack_size = 128 * 4, .priority = (osPriority_t) osPriorityLow, };
/* Definitions for dfuTask */
osThreadId_t dfuTaskHandle;
const osThreadAttr_t dfuTask_attributes = { .name = "dfuTask", .stack_size = 128
        * 4, .priority = (osPriority_t) osPriorityLow, };
/* Definitions for tickQueue */
osMessageQueueId_t tickQueueHandle;
const osMessageQueueAttr_t tickQueue_attributes = { .name = "tickQueue" };
/* Definitions for wasmCtlQueue */
osMessageQueueId_t wasmCtlQueueHandle;
const osMessageQueueAttr_t wasmCtlQueue_attributes = { .name = "wasmCtlQueue" };
...
const osMutexAttr_t ws2812Mutex_attributes = { .name = "ws2812Mutex" };
/* Definitions for ledSemaphore */
osSemaphoreId_t ledSemaphoreHandle;
const osSemaphoreAttr_t ledSemaphore_attributes = { .name = "ledSemaphore" };
/* USER CODE BEGIN PV */

// Let's define our wasm task
osThreadId_t wasmTaskHandle;
const osThreadAttr_t wasmTask_attributes = { .name = "wasmTask", .stack_size = 4
...
void startWasmCtlTask(void *argument);
...
int main(void) {
    ...
    tickQueueHandle = osMessageQueueNew(3, sizeof(uint32_t),
            &tickQueue_attributes);

    /* creation of wasmCtlQueue */
    wasmCtlQueueHandle = osMessageQueueNew(2, sizeof(uint16_t),
            &wasmCtlQueue_attributes);
    ...
    /* creation of wasmCtlTask */
    wasmCtlTaskHandle = osThreadNew(startWasmCtlTask, NULL,
            &wasmCtlTask_attributes);

    /* creation of dfuTask */
    dfuTaskHandle = osThreadNew(startDfuTask, NULL, &dfuTask_attributes);
...
void startWasmTask(void *argument) {
    ...
    if (wasm_file_size >= 8 && wasm_buffer[0] == 0x00 && wasm_buffer[1] == 0x61) {
        ...
        if (env) {
            // Allocate runtime with enough overhead for the Wasm page
            ...
            if (runtime) {
                ...
                if (result != m3Err_none && result != m3Err_trapAbort) {
                    if (osMutexAcquire(printMutexHandle, 100) == osOK) {
                        printf("Wasm: Error: %s\n", result);
                        osMutexRelease(printMutexHandle);
                    }
...
/* USER CODE BEGIN Header_startWasmCtlTask */
/**
 * @brief Function implementing the wasmCtlTask thread.
 * @param argument: Not used
 * @retval None
 */
/* USER CODE END Header_startWasmCtlTask */
void startWasmCtlTask(void *argument) {
    /* USER CODE BEGIN startWasmCtlTask */
    uint16_t wasm_ctl_cmd;

    for (;;) {
        osMessageQueueGet(wasmCtlQueueHandle, &wasm_ctl_cmd, NULL,
        osWaitForever);

        osMutexWait(printMutexHandle, osWaitForever);
        if (wasm_ctl_cmd == 1) { // DFU Start / Stop Signal
            printf("WasmCtl: Stopping task for DFU...\n");

            if (wasmTaskHandle != NULL) {
                wasm_should_stop = true;

                // Wait for the Wasm task to hit host_delay and self-terminate
                uint8_t timeout = 0;
                while (wasmTaskHandle != NULL && timeout < 50) {
                    osDelay(10);
                    timeout++;
                }

                // Force delete only if the trap failed (e.g. tight loop in Wasm)
                if (wasmTaskHandle != NULL) {
                    printf("WasmCtl: Force deleting stuck task\n");
                    vTaskDelete(wasmTaskHandle);
                    wasmTaskHandle = NULL;
                }
            }
            wasm_should_stop = false;

        } else if (wasm_ctl_cmd == 2) { // DFU Manifest / Start Signal
            printf("WasmCtl: Starting new Wasm instance...\n");

            wasmTaskHandle = osThreadNew(startWasmTask, NULL,
                    &wasmTask_attributes);

            if (!wasmTaskHandle) {
                printf("WasmCtl: ERROR - Failed to start task\n");
            }
        }
        osMutexRelease(printMutexHandle);
    }
...
/* USER CODE BEGIN Header_startDfuTask */
/**
 * @brief Function implementing the dfuTask thread.
 * @param argument: Not used
 * @retval None
 */
/* USER CODE END Header_startDfuTask */
void startDfuTask(void *argument) {
    /* USER CODE BEGIN startDfuTask */
